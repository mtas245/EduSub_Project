        ...
        with ui.row().classes('gap-2'):
            ...
            ui.button('Logout', icon='logout',
                      on_click=lambda: (
                          ...
                          ui.navigate.to('/')
                      )).classes('bg-transparent text-white border border-blue-500 rounded-lg px-4 py-2 text-sm hover:bg-blue-800')
            ...
            with ui.tab_panel(tab_open).classes('px-0'):
                ...
                with ui.row().classes('items-center gap-3 py-4'):
                    ...
                    count_label = ui.label('').classes(
                        'ml-auto text-sm text-gray-400 bg-gray-100 px-3 py-1 rounded-full')
                ...
                def render_open():
                    ...
                    count_label.set_text(
                        f'{len(filtered)} assignment{"s" if len(filtered) != 1 else ""}')
                    ...
                    with open_container:
                        if not filtered:
                            with ui.column().classes('w-full items-center py-20 gap-3'):
                                ...
                                ui.label('No assignments available').classes(
                                    'text-base font-medium text-gray-600')
                                ui.label(
                                    'There are no open substitute requests right now. '
                                    'Check back later or adjust the filter.').classes(
                                    'text-sm text-gray-400 text-center max-w-xs')
                                with ui.row().classes('items-center gap-1 text-xs text-gray-300 mt-1'):
                                    ui.icon('notifications').classes('text-sm')
                                    ui.label('New assignments will appear here automatically')
                            return
                        ...
                        for req in filtered:
                            ...
                            accent = 'bg-gray-300' if already_applied else (
                                'bg-green-500' if is_kg else 'bg-blue-600')
                            grade_badge = 'bg-green-50 text-green-700' if is_kg else 'bg-blue-50 text-blue-700'
                                ...
                                with ui.row().classes('w-full items-stretch'):
                                    ...
                                    with ui.column().classes('flex-1 p-4 gap-2'):
                                        ...
                                        with ui.row().classes('w-full justify-between items-center'):
                                            ui.label(subject_name).classes(
                                                'text-base font-semibold text-gray-800')
                                            ui.label(req.grade_level).classes(
                                                f'text-xs font-semibold px-3 py-1 rounded-full {grade_badge}')

                                        # Meta row
                                        with ui.row().classes('gap-4 text-sm text-gray-500 flex-wrap'):
                                            with ui.row().classes('items-center gap-1'):
                                                ui.icon('calendar_today').classes('text-sm text-blue-400')
                                        ...
                                        with ui.row().classes('w-full justify-between items-center mt-1'):
                                            ...
                                            if already_applied:
                                                with ui.row().classes(
                                                        'items-center gap-1 text-sm text-green-600 font-medium'):
            ...
            with ui.tab_panel(tab_mine).classes('px-0'):
                ...
                if not my_assignments:
                    with ui.column().classes('w-full items-center py-20 gap-3'):
                        ...
                        ui.label('No confirmed assignments yet').classes(
                            'text-base font-medium text-gray-600')
                        ui.label(
                            'Once an admin approves your application, '
                            'your assignments will appear here.').classes(
                            'text-sm text-gray-400 text-center max-w-xs')
                else:
                    with ui.row().classes('items-center gap-2 py-4'):
                        ui.icon('check_circle').classes('text-green-500 text-lg')
                        ui.label(
                            f'{len(my_assignments)} confirmed '
                            f'assignment{"s" if len(my_assignments) != 1 else ""}'
                                    ...
                                    with ui.column().classes('flex-1 p-4 gap-2'):
                                        with ui.row().classes('w-full justify-between items-center'):
                                            ...
                                            with ui.row().classes('items-center gap-2'):
                                                ui.label(req.grade_level).classes(
                                                    f'text-xs font-semibold px-3 py-1 rounded-full {grade_badge}')
                                                ui.badge('Confirmed', color='green').classes(
                                                    'text-white text-xs')

                                        with ui.row().classes('gap-4 text-sm text-gray-500 flex-wrap'):
                                            with ui.row().classes('items-center gap-1'):
                                                ui.icon('calendar_today').classes('text-sm text-green-400')
                                                ui.label(req.date.strftime('%d.%m.%Y'))
                                            if req.time_slot:
                                                with ui.row().classes('items-center gap-1'):
                                            ...
                                            with ui.row().classes('items-center gap-1'):
                                                ...
                                                ui.label(
                                                    f'{SCHOOL_NAME} · {SCHOOL_ADDRESS}').classes(
