...
def admin_dashboard():
    ...
    with ui.row().classes('w-full justify-between items-center p-4 bg-blue-800 text-white'):
    ...
    with ui.column().classes('w-full p-6 gap-6'):
        ...
        with ui.row().classes('w-full gap-4'):
            with ui.card().classes('p-4 min-w-32 text-center'):
                ...
                ui.label('Total Requests').classes('text-gray-500')
            with ui.card().classes('p-4 min-w-32 text-center'):
                ui.label(str(len(open_requests))).classes('text-3xl font-bold text-green-700')
                ui.label('Open Requests').classes('text-gray-500')
            with ui.card().classes('p-4 min-w-32 text-center'):
                ui.label(str(len(pending_apps))).classes('text-3xl font-bold text-yellow-700')
                ui.label('Pending Applications').classes('text-gray-500')
                ...
                for appl in pending_apps:
                    ...
                    with ui.row().classes('items-center gap-4 border-b py-2'):
                        with ui.column().classes('flex-1'):
                            ui.label(f'Request ID: {appl.request_id}').classes('text-sm text-gray-500')
                            with ui.row().classes('items-center gap-2'):
                                ui.label('Teacher:').classes('text-sm text-gray-500')
                                ui.link(
                                    f'{teacher_pnr} ({teacher_name})',
                                    f'/admin/teacher/{appl.teacher_id}'
                                ).classes('font-mono font-bold text-blue-700 hover:underline')
                        ui.label(
                            f'Applied: {appl.applied_at.strftime("%Y-%m-%d %H:%M") if appl.applied_at else "-"}'
                        ).classes('flex-1 text-sm text-gray-500')
                        app_id = appl.id
                        ui.button(
                            'Approve',
